...
// 实例化状态机 (Timer)
   Timer timer (
    ...
    blk_mem_gen_0 data_memory (
        ...
        .clka(clk),                         // 时钟信号
        .ena(mem_en),                       // 访存使能信号
        .wea(mem_wea),                      // 数据存储器写使能信号
        .addra(mem_addr[16:2]),              // 数据访问地址（低 10 位）
        .dina(mem_write_data),              // 数据写入数据
        
        // 端口B：用于读取数据（从数据存储器中读取数据）
        .clkb(clk),                         // 时钟信号
        .enb(mem_en),                         // 使能信号，始终使能读取数据
        .addrb(mem_addr[16:2]),              // 数据访问地址
        .doutb(read_data)                   // 输出读出数据
    );
